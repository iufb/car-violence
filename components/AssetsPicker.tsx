...
import { DeviceEventEmitter, Dimensions, Image, Modal, Pressable, StyleSheet, View } from "react-native";
...
export const AssetsPicker = () => {
    ...
    return visible && <GestureHandlerRootView><Modal visible={visible} transparent={true} animationType="slide" onRequestClose={() => setVisible(false)}>
        <View style={[styles.container]}>
            ...
            {activeTab == tabs[0] ? <PhotosView /> : <VideosView />}
        </View>
...
const PhotosView = ({ }: AssetsPickerBase) => {
    ...
    return <View>
        ...
        <View>
            {photos.map(photo =>
                <Image key={photo.id} source={{ uri: photo.uri }} width={100} height={100} />
            )}
        </View>
    </View>
}
